    ...
    try {
      await (async () => {
        ...
        wsocket = makeWASocket({
          ...
          userDevicesCache,
          shouldIgnoreJid: (jid) => { return isJidBroadcast(jid) || isJidStatusBroadcast(jid) || jid.includes('newsletter') },
        ...
        if (whatsappUpdate?.importOldMessages) {
          ...
          let endImportDate = new Date(whatsappUpdate.importRecentMessages).getTime()
